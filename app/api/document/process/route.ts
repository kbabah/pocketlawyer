...
async function extractTextFromPDF(buffer: Buffer, options?: { language?: string }): Promise<string> {
  try {
    ...
    try {
      ...
      logger.error('OCR process failed:', error.message);
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (file.size > MAX_FILE_SIZE) {
        return NextResponse.json({
          error: `The file exceeds the 5MB limit.`,
          success: false
        }, { status: 400 });
      }
      ...
      logger.error('Document processing error:', innerError);
      return NextResponse.json({ 
    ...
    logger.error('Critical API error:', outerError);
    return NextResponse.json({ 
